...
%         sigma = ak_large_datasets_get_proper_rbf_sigma(X_Train, size(X_Train,1));
        sigma = 20;
        XX = [X_Train ; X_Test];
        K = ak_fast_cross_rbf_kernel(XX,XX,sigma);
        K = (K+K')/2;
        Phi = cholcov(K);
        Phi = Phi';
        Phi = Phi(:, 1:size(X_Train,1));
        
        X_Train = Phi(1:size(X_Train,1),:);
        X_Test = Phi(size(X_Train,1)+1:size(Phi,1),:);
    end
...
%     PXX = PX' * PX;
    ...
    for iter=1:numberOfIters
        ...
        C_new = C_old + eng*(2*alpha*PX*C_old - 2*beta*C_old*(D_old*D_old')+ 2*beta*Y_Train*D_old'); 
